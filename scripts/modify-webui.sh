...
TMP_WWW_PATH="$TMP_DIR/$script_name" # directory to store modified files in
...
sed_and_check() {
    ...
    logger -st "$script_name" "Failed to apply modification to '$(basename "$4")':  '$1'  '$2'  '$3'"
    ...
    case "$1" in
        "set")
            ...
            if ! mount | grep -Fq /www/device-map/router_status.asp; then
                ...
                sed_and_check append 'Object.keys(cpu_info_new).length;' '$("#cpu_temp").html(parseFloat(cpu_temp_new).toFixed(1));' "$TMP_WWW_PATH/device-map/router_status.asp"
                sed_and_check prepend "('#cpu_field').html(code);" "code += '<div class=\"info-block\">Temperature: <span id=\"cpu_temp\"></span> °C</div>';" "$TMP_WWW_PATH/device-map/router_status.asp"
    ...
    case "$1" in
        "set")
            if ! mount | grep -Fq /www/Guest_network.asp; then
                ...
                sed_and_check append '<script type="text/javascript" src="js/httpApi.js"></script>' '<script src="https://cdn.rawgit.com/davidshimjs/qrcodejs/gh-pages/qrcode.min.js"></script>' "$TMP_WWW_PATH/Guest_network.asp"
                sed_and_check append 'onclick="applyRule();">' '<br><span id="qr_code" style="display:inline-block;margin:25px 0 25px 0;"></span>' "$TMP_WWW_PATH/Guest_network.asp"
    ...
    case "$1" in
        "set")
            if ! mount | grep -Fq /www/state.js; then
                ...
                sed_and_check append 'var lyra_hide_support = isSupport("lyra_hide")' 'var notrendmicro_support = isSupport("notrendmicro");' "$TMP_WWW_PATH/state.js"
            ...
            if ! mount | grep -Fq /www/require/modules/menuTree.js || ! grep -Fq 'notrendmicro_support' /www/require/modules/menuTree.js; then
                ...
                if [ -n "$inetspeed_tab" ]; then
                    ...
                    sed_and_check append '{url: "Advanced_Smart_Connect.asp' "$inetspeed_tab" "$TMP_WWW_PATH/require/modules/menuTree.js"
                else
        ...
        "unset")
            if [ -n "$merlin" ] || [ -f /tmp/menuTree.js ]; then
                logger -st "$script_name" "Unable to revert 'notrendmicro_support' tweak on Asuswrt-Merlin firmware - reboot is required!"
                return
            fi
