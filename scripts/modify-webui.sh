...
sed_and_check() {
    ...
    logecho "Failed to apply modification to '$(basename "$4")':  '$1'  '$2'  '$3'"
    ...
    case "$1" in
        "set")
            ...
            if ! mount | grep -Fq /www/require/modules/menuTree.js || ! grep -Fq 'notrendmicro_support' /www/require/modules/menuTree.js; then
                ...
                if [ -n "$inetspeed_tab" ]; then
                    ...
                    logecho "There was a problem performing modification on file '$TMP_WWW_DIR/require/modules/menuTree.js': unable to find line containing 'AdaptiveQoS_InternetSpeed'!"
                fi
        ...
        "unset")
            if [ -n "$merlin" ] || [ -f /tmp/menuTree.js ]; then
                logecho "Unable to revert 'notrendmicro_support' tweak on Asuswrt-Merlin firmware - reboot is required!"
                return
            fi
...
www_override() {
    case "$1" in
        "set")
            [ -z "$TWEAKS" ] && { logecho "Error: No tweaks to apply"; exit 1; }
            ...
            [ -n "$applied" ] && logecho "Applied WebUI tweaks: $(echo "$applied" | awk '{$1=$1};1')" true
        ;;
        "unset")
            logecho "Removing WebUI tweaks..." true
